...
public class ClasspathService {
    ...
    public List<ClasspathEntry> getClasspath(@QueryParam("projectpath") String projectPath) throws JavaModelException {
        IJavaProject javaProject = model.getJavaProject(projectPath);
        return convertToClasspathEntryDTO(javaProject.getRawClasspath());
    }

    private List<ClasspathEntry> convertToClasspathEntryDTO(IClasspathEntry[] entries) {
        if (entries.length == 0) {
            return emptyList();
        }
        ...
        for (IClasspathEntry entry : entries) {
            ClasspathEntry entryDTO = DtoFactory.getInstance().createDto(ClasspathEntry.class);
