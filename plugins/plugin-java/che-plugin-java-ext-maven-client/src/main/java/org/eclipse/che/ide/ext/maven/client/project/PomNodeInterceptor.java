 ...
 *******************************************************************************/
package org.eclipse.che.ide.ext.maven.client.project;
...
import org.eclipse.che.ide.ext.maven.client.MavenResources;
...
public class PomNodeInterceptor implements NodeInterceptor {
    ...
    public Promise<List<Node>> intercept(Node parent, List<Node> children) {
        for (Node child : children) {
            if (child instanceof FileReferenceNode && "pom.xml".equals(child.getName())) {
                ...
                pom.getAttributes()
                   .put(DISPLAY_NAME_ATTR, singletonList(attributes.containsKey(ARTIFACT_ID)
    ...
    public int getPriority() {
        return NodeInterceptor.MAX_PRIORITY;
    }
}
