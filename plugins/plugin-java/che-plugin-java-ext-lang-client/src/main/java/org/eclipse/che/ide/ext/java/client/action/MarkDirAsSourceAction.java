...
public class MarkDirAsSourceAction extends AbstractPerspectiveAction {
    ...
    private void updateClasspath(final CurrentProject currentProject, final FolderReferenceNode folder) {
        classpathService.getClasspath(currentProject.getProjectConfig().getPath()).then(new Operation<List<ClasspathEntry>>() {
            @Override
            ...
            public void apply(PromiseError arg) throws OperationException {
                notificationManager.notify("Can't get classpath", arg.getMessage(), FAIL, true);
            }
        });
    }
}
