...
public class ProjectClasspathPresenterTest {
    ...
    @Mock
    private Promise<List<ClasspathEntry>>                   promise;
    @Captor
    private ArgumentCaptor<Operation<List<ClasspathEntry>>> acceptCaptor;
    @Captor
    ...
    public void showConfirmDialogIfHaveUnsavedChanges() throws Exception {
        ...
        verify(locale).buttonSaveChangesDiscard();
    }
    ...
    @Test
    public void revertAllChangesIfCancelButtonWasClicked() throws Exception {
        when(page1.isDirty()).thenReturn(true);
        when(page2.isDirty()).thenReturn(true);

        presenter.onCancelClicked();

        verify(page1).revertChanges();
        verify(page2).revertChanges();
    }
    ...
    @Test
    public void changeStateOfButtonsIfDirtyStateWasChanged() throws Exception {
        when(page1.isDirty()).thenReturn(true);

        presenter.onDirtyChanged();

        verify(view).setSaveButtonState(true);
        verify(view).setCancelButtonState(true);
    }

    @Test
    public void changeStateOfButtonsIfDirtyStateWasChanged2() throws Exception {
        presenter.onDirtyChanged();
    ...
    public void windowShouldBeShowedWithoutErrors() throws Exception {
        List<ClasspathEntry> arg = new ArrayList<>();
        when(projectConfigDto.getPath()).thenReturn("path");
        when(service.getClasspath(anyString())).thenReturn(promise);
        when(promise.then(Matchers.<Operation<List<ClasspathEntry>>>anyObject())).thenReturn(promise);
    ...
    public void showErrorNotificationIfReadingClasspathHasError() throws Exception {
        ...
        verify(notificationManager).notify("Problems with getting classpath", TEXT, FAIL, true);
    }
}
