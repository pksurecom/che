...
public class JGitConnectionFactory extends GitConnectionFactory {
    ...
    @Inject
    public JGitConnectionFactory(CredentialsLoader credentialsLoader, GitUserResolver userResolver) throws GitException {
    ...
    public JGitConnection getConnection(File workDir, LineConsumerFactory outputPublisherFactory) throws GitException {
        return getConnection(workDir, userResolver, outputPublisherFactory);
    }

    public JGitConnection getConnection(File workDir, GitUserResolver gitUserResolver, LineConsumerFactory outputPublisherFactory)
            throws GitException {
        Repository gitRepo = createRepository(workDir);
        JGitConnection conn = new JGitConnection(gitRepo, gitUserResolver.getUser(), credentialsLoader);
